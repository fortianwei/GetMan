...
### 进程管理（重要）
- **禁止**使用 `taskkill /F /IM node.exe` 等全局杀进程命令，会影响其他正在运行的项目
- 端口冲突时，先用 `netstat -ano | findstr :<端口>` 查找具体占用的 PID，只处理该 PID
- 启动服务前先检查端口是否可用，有冲突时告知用户，由用户决定如何处理
- 始终采用温和、精准的方式管理进程，避免误杀其他项目的进程
